...
|----|------|---------|--------|-----------|
| L4 前端 | DocumentEditor | Yjs块级协同编辑 | REST+WS | BC1+BC2 |
| L4 前端 | ForgeStation | 润色台/提案Diff/AI交互 | REST | BC3 |
| L4 前端 | ArbitrationView | 冲突仲裁可视化 | WS | BC4 |
| L4 前端 | ApprovalPanel | 审批三选项/决策面板 | REST | BC4 |
| L4 前端 | RoleDashboard | 角色管理/权限/认领视图 | REST | BC5 |
...
```
L4 前端 ──→ L3 API/WS ──→ L2 Application ──→ L2 Domain ──→ L1 Infra
                                                 ↘ Audit ←── ALL
```
...
|--------|------|------|--------|------|---------|
| m1 | DocumentEditor | module | L4 | 组件拆分 | DDD BC2 |
